...
- Paquetes activos historicos: `pi-rtk-optimizer`, `pi-codemapper`, `pi-minimal-subagent`, `pi-observational-memory`, `pi-web-access`, `pi-mcp-extension`, `pi-until-done`, `pi-chrome`, `pi-council`, `@calesennett/pi-codex-usage`, `@mporenta/pi-discord-remote`, `@juicesharp/rpiv-ask-user-question`, `@juicesharp/rpiv-i18n`, `@venthezone/picord`, `pi-ask-user`, `pi-footer`, `pi-agents-pool`. `@llblab/pi-telegram` fue removido porque JP ya no lo usa.
...
- Config Pi alineada con Windows salvo diferencias OS-specific:
  ...
  - paquetes principales: `pi-rtk-optimizer`, `pi-codemapper`, `pi-minimal-subagent`, `pi-observational-memory`, `pi-web-access`, `pi-mcp-extension`, `pi-until-done`, `pi-council`, `pi-codex-usage`, `pi-discord-remote`, `rpiv-ask-user-question`, `rpiv-i18n`, `pi-ask-user`, `pi-footer`, `pi-agents-pool`, `pi-computer-use`. `pi-chrome` es OS/browser-specific y no debe sincronizarse al VPS salvo pedido explicito. `@llblab/pi-telegram` fue removido del VPS porque JP ya no lo usa y emitia `Theme not initialized` en PI WEB/RPC.
