...
2. Verificar path, git status y capas OS baratas del target. Si el path falta, tratarlo como `unavailable locally`: bloquear y pedir ruta/sync; no crear carpeta nueva ni degradar el estado durable del registry sin confirmacion.
3. Si hay varios candidatos o capas ambiguas, en Pi preferir `ask_user` multi-select.
