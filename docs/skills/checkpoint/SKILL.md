---
name: checkpoint
description: "Legacy alias for `guardar sesion` - persist durable value from the current session into repo docs without opening a new session, handoff, gol, or compaction. Use when the user says `checkpoint` or `persistí estado`."
---
