---
name: guardar-sesion
description: Persist the valuable durable parts of the current session into the right repo docs without opening a new session or changing the active goal. Use when JP says `guardar sesion`, `documentar sesion`, `checkpoint`, or wants to make sure the session's value is preserved.
---
