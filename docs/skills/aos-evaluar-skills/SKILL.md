...
4. For broad audits, use `taskflow` reviewers or a read-only worker to summarize many skills instead of dumping every long `SKILL.md` into the main context.
5. Classify each candidate as `skill`, `hybrid skill`, `topic`, `active rule`, `track`, or `do not promote`.
6. Produce a shortlist with reason, trigger phrase, canonical source, and risk/cost. Mention when `advisor`, Ponytail, `dgoal`, `pi-context-viewer`, `pi-lens`, `taskflow`, `pi-task`, `pi-code-planner`, `pi-long-task`, Plannotator, or `pi-simplify` should be part of that command's flow.
7. If JP asks to implement, create or update only `docs/skills/<name>/`; keep durable logic in topics/scripts/docs and avoid duplicating long procedures.
8. Run `scripts/ensure-skills-link.ps1`, validate changed skills, regenerate the context index, and run the audit.
